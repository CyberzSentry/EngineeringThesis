...
\begin{itemize}
   \item IP addresses - numerical label assigned to each device connected to a computer network that uses the Internet Protocol for communication.
   \item Personal identification numbers - tokes issued by countries to citizens for identification purposes, e.g., Social Security Number, PESEL. 
   \item ID card numbers - serial numbers of phisical identification cards issued to country citizens.
   \item MAC addresses - unique identifiers assigned to a network interface controller for use as a network address in communications within a network segment.
   \item Domain names - an identification strings that define a realm of administrative autonomy, authority or control within the Internet. Domain names are used
    in various networking contexts and for application-specific naming and addressing purposes.
   \item Email addresses - identifiers of an email box to which email messages are delivered.
   \item Passwords - secrets used to confirm the identity of a user.
   \item Usernames - a unique sequence of characters used by a person with access to a computer, network, or online service.
   \item Phone numbers - sequence of digits assigned to a fixed-line telephone subscriber station connected to a telephone line or to a wireless electronic telephony 
   device, such as a radio telephone or a mobile telephone, or to other devices for data transmission via the public switched telephone network (PSTN) or other 
...
An example of this is a personal identification number which may vary depending on issuing authority. Giving the user a accessible possibility to 
