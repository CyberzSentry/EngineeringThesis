...
a large amount of natural language messages in search of data that can be classified as sensitive. Special focus is 
put on possibility of calibration and customisation of the search engine and it's reusability, regardless of 
circumstances and enviroment.

This thesis however does not elaborate the process of defining a proper sensitive data structure outside of the scope 
...
There are multiple important project assumptions that have to be thought through before the design and 
implementation of the project. 

\section{Sensitive data definition}

The project will focus on finding nine categories of information:

\begin{itemize}
   \item IP addresses
   \item Social security numbers
   \item ID card numbers
   \item MAC addresses
   \item Domains
...
Due to the random nature of those categories generating a dictionary for search purposes can prove difficult or in some cases
even impossible. The complexity of this task would be further increased because of the different ways that some of those
data types can be presented. In example an ip address can be stated either as "127.0.0.1" or "127 0 0 1" with the same meaning
for the recipient of the message. 

There is exists however a tool called "regural expresions" that is frequently used for parsing users input and capturing 
parts of strings. \cite{bib:conferenceRegex} This natural language processing tool is capable of taking under consideration 
fluctuations in the message formulation. 

A dictionary search could however prove useful in searching for specific keywords that could indicate a presence of searched
information. Such words like "login" or "password".

It might be advantageous for the purposes of this project to take under consideration additional properties of the considered 
data types. It is common for the personal information issued by some entities to implement a check sum algorithm 
used for validation of authenticity. Implementing a adjustable additional check of the data discovered by regular expressions 
could possibly reduce the amount of false positives. Such feature could be utilised by individual user to further enhance 
accuracy in any case where identifing additional patterns, that escape regular expression domain, is possible.
...
In order to mitigate those problems a modular approach to the design could be taken that would allow for 


\section{Enviroment independence}
